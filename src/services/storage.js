...
export async function dbGet(key, def) {
  if (useLS) { const d = JSON.parse(ls()); return key in d ? d[key] : def }
  const v = await idbKeyVal.get(key);
  return v !== undefined ? v : def;
}

export async function dbSet(key, val) {
  if (useLS) { const d = JSON.parse(ls()); d[key] = val; lss(JSON.stringify(d)); return }
  await idbKeyVal.set(key, val);
}

export async function dbDel(key) {
  if (useLS) { const d = JSON.parse(ls()); delete d[key]; lss(JSON.stringify(d)); return }
  await idbKeyVal.del(key);
}

export async function dbKeys() {
  if (useLS) return Object.keys(JSON.parse(ls()));
